...
def dashboard(request):
    ...
    for event in events:
        ...
        if event.eventDate < datetime.now().date():
            previousEvents+=1
    ...
    if type == "totalevents":
        ...
        contextData = events.filter(eventDate__lt=date.today())
    
    elif type == "upcomingevents":
        contextData = events.filter(eventDate__gt=date.today())
    
    else:
        #todaysevents:
        contextData = events.filter(eventDate = date.today())
